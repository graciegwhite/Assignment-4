...
traps_line_g <- ggplot(trap5_sumtable, aes(x = YEAR, y = mean_traps, group = SITE, color = SITE)) +
  ...
  labs(x = "Year", y = "Observed Traps", title = "Mean Annual Observed Lobster Traps at 3 Non-MPA LTER Sites (2012-2017)") + scale_colour_brewer(palette = "YlOrRd")
...
lsize_2017
```
####Table 1. Lobster Size at 5 LTER Sites in the Santa Barbara Channel, 2017
Sample means of measured carapce lengths ± 1 standard deviation (mm) of lobsters in 2017. Count represents the number of lobsters measured at each site. Like letters indicate means that do not differ significantly by one-way ANOVA (F(4) = 3.424, p = 0.0085, $\alpha$ = 0.05) with Tukey's HSD ($\alpha$ = 0.05). 
...
lobs_size_table
```
####Table 2. Change in Lobster Size at 5 LTER Sites in the Santa Barbara Channel between 2012 and 2017.
Sample means of measured carapce lengths ± 1 standard deviation (mm) of lobsters in 2012 and 2017. The number of lobsters measured at each site in each year is listed as (n = ss). Two-Sample Student T-Tests were performed comparing mean lobster size at each site in 2012 and 2017. The only significant difference in mean lobster size from 2012 to 2017 was found at MOHK (t(259) = 4.069, p <.001, $\alpha$ = 0.05) The actual change in mean carapace length (mm) is described in the "difference" column.  
...
prop_table2 <- kable(legaltable2, col.names = c(" ", "Count (Lobsters)", "Illegal (< 82.6 mm)", "Legal (> 82.6 mm)"), align = 'c') %>% 
  kable_styling(font_size = 11, "condensed") %>% 
  group_rows("MPA", 1, 2) %>% 
  group_rows("Non-MPA", 3, 5) %>% 
...
Statement 1
Fishing pressure is decreasing over time, and severly decreased in 2017. There is a spike in lobster abundance in 2015, and a drop in 2016. 2017 lobster abundance then increases again. 
Statement 2
By site lobster sizes in 2017. NAPL has the largest mean lobster size. A one-way anova was performed to compare mean carapass length in lobsters across sites. There was a significant difference. A post-hoc TUKEY HSD was performed to make pairwise comaparisons. NAPL-IVEE and NAPL-CARP had significantly different means. 
Statement 3
Mean lobster sizes were compared for each site between 2012 and 2017. F tests for equal variances were ran (equal for all) and thus student t-tests were used to compare means across years. Effect size and power tests were also ran for each t-test. The only significant result for a difference in mean size between the two years is at MOHK. A one-way t-test was ran and sigfnicant that mean lobster size decreased in 2017 from 2012. While not statistically significant, the observed sample means show a small increase in lobster size in the MPA sites  & AQUE, and a decrease in mean lobster size at CARP and MOHK (mohk is stat. sig.)
Statement 4
Compared proportions of legal vs. illegal lobsters (based on carapace length) at each site in 2017. Proportions between legal and illegal lobster size is significantly different at MOHK and NAPL. At MOHK, 87% of lobsters observed were smaller than the legal limit. At NAPL, only 66% of lobsters were smaller than the legal limit. The other three sites, saw 74-78% of the lobsters smaller than the legal size limit. A chi-square test for proportions and compared standardized residuals. 
